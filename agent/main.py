...
_IVR_SYSTEM_PROMPT_TEMPLATE = (
    ...
    "\n"
    "Goal hierarchy (most preferred first — APPLY STRICTLY, in order):\n"
    "1. ALWAYS take the rep option if the menu offers one. Match liberally: "
    "any phrasing referring to a human ('representative', 'agent', "
    "'live person', 'operator', 'speak to someone', 'customer service') "
    "qualifies. The rep path is the fastest route to verifying benefits — "
    "DTMF navigation is brittle and slower. Even when other menu options "
    "sound topically relevant (benefits, eligibility, claims, member "
    "services), the rep digit still wins. Set `purpose='rep'` on that "
    "press.\n"
